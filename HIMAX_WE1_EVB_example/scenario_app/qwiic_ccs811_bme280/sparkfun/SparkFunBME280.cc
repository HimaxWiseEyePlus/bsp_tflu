...
void BME280::readRegisterRegion(uint8_t *outputPointer , uint8_t offset, uint8_t length)
{
	...
	hx_drv_i2cm_set_data(settings.I2CAddress,regAddr, 1, 0, 0);
	hx_drv_i2cm_get_data(settings.I2CAddress,NULL,0,outputPointer,length);
	#else
	uint8_t i = 0;
	char c = 0;
...
uint8_t BME280::readRegister(uint8_t offset)
{
	...
	uint8_t regAddr[1] = {offset};
	hx_drv_i2cm_set_data(settings.I2CAddress,regAddr, 1, 0, 0);
	hx_drv_i2cm_get_data(settings.I2CAddress,NULL,0,&result,numBytes);
	#else
	switch (settings.commInterface) {
...
void BME280::writeRegister(uint8_t offset, uint8_t dataToWrite)
{
	#ifdef HIMAX_BME280
	uint8_t regAddr[1] = {offset};
	hx_drv_i2cm_set_data(settings.I2CAddress,regAddr, 1, &dataToWrite, 1);
	#else
	switch (settings.commInterface)
	{
	case I2C_MODE:
