...
#ifdef HIMAX_CCS811
CCS811Core::CCS811_Status_e CCS811Core::beginCore()
{
...
CCS811Core::CCS811_Status_e CCS811Core::readRegister(uint8_t offset, uint8_t *outputPointer)
{
	...
	uint8_t numBytes = 1;
#ifdef HIMAX_CCS811
	uint8_t regAddr[1] = {offset};
	if (hx_drv_i2cm_set_data(I2CAddress, regAddr, 1, 0, 0) != HX_DRV_LIB_PASS)
	{
		returnError = CCS811_Stat_I2C_ERROR;
		return returnError;
	}
	if (hx_drv_i2cm_get_data(I2CAddress, NULL, 0, outputPointer, numBytes) != HX_DRV_LIB_PASS)
	{
		returnError = CCS811_Stat_I2C_ERROR;
		return returnError;
	}
...
CCS811Core::CCS811_Status_e CCS811Core::multiReadRegister(uint8_t offset, uint8_t *outputPointer, uint8_t length)
{
	...
	if (hx_drv_i2cm_get_data(I2CAddress, NULL, 0, outputPointer, length) != HX_DRV_LIB_PASS)
	{
		returnError = CCS811_Stat_I2C_ERROR;
		return returnError;
	}
...
#ifdef HIMAX_CCS811
	uint8_t regAddr[1] = {offset};
	if (hx_drv_i2cm_set_data(I2CAddress, regAddr, 1, &dataToWrite, 1) != HX_DRV_LIB_PASS)
	{
		returnError = CCS811_Stat_I2C_ERROR;
		return returnError;
	}
...
#ifdef HIMAX_CCS811
	uint8_t regAddr[1] = {offset};
	if (hx_drv_i2cm_set_data(I2CAddress, regAddr, 1, inputPointer, length) != HX_DRV_LIB_PASS)
	{
		returnError = CCS811_Stat_I2C_ERROR;
		return returnError;
	}
...
bool CCS811::begin()
{
	if (beginWithStatus() == CCS811_Stat_SUCCESS)
		return true;
	return false;
}
#else
...
//****************************************************************************//
#ifdef HIMAX_CCS811
CCS811Core::CCS811_Status_e CCS811::beginWithStatus()
{
	...
	uint8_t regAddr[1] = {CSS811_APP_START};
	if(hx_drv_i2cm_set_data(I2CAddress,regAddr,1,0,0)!= HX_DRV_LIB_PASS){
...
CCS811Core::CCS811_Status_e CCS811::readNTC(void)
{
	...
	//hx_drv_uart_print("vrefCounts: ");
	//hx_drv_uart_println(vrefCounts);
	ntcCounts = ((uint16_t)data[2] << 8) | data[3];
	//hx_drv_uart_print("ntcCounts: ");
	//hx_drv_uart_println(ntcCounts);
	//hx_drv_uart_print("sum: ");
	//hx_drv_uart_println(ntcCounts + vrefCounts);
